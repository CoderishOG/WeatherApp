...
// Make an initial request to the API with a default city name and store the response data
// let defaultCity = 'London';
// let defaultData;
// requests(`https://api.openweathermap.org/data/2.5/weather?q=raipur&appid=988e15380dd0eb226e5767fde70e2a84`)
//   .on('data', (chunk) => {
...
//       if (req.url === '/') {
//         // If the request is for the home page, send the HTML template with the default data
//         res.writeHead(200, { 'Content-type': 'text/html' });
//         res.end(defaultData);
        if (req.method === 'POST') {
        // If the request is for getting weather data for a new city, parse the request body and make a new API request
        let body = '';
        req.on('data', chunk => {
          body += chunk.toString();
        });
...
requests(`https://api.openweathermap.org/data/2.5/weather?q=raipur&units=metric&appid=988e15380dd0eb226e5767fde70e2a84`)
  ...
  .on('end',()=>{
        ...
        // If the request is for any other page, return a 404 error
